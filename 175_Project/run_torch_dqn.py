...
def genMap():
    ...
    i = 2
    w = genString(1,i,2, 'water')
    mapStr += w
    w = genString(1,i,3, 'water') #
    mapStr += w    
    w = genString(2,i,1, 'water')
    mapStr += w    
    w = genString(2,i,3, 'water')
    mapStr += w  
    w = genString(3,i,1, 'water') #
    mapStr += w     
    w = genString(3,i,2, 'water')
    mapStr += w         
    w = genString(1,i,1, 'water') #
    mapStr += w
    w = genString(2,i,2, 'water') #
    mapStr += w    
    w = genString(3,i,3, 'water') #
    mapStr += w        
    ...
    i = 0
    w = genString(1,i,2, blocktype)
    mapStr += w
    w = genString(1,i,3, blocktype) #
    mapStr += w    
    w = genString(2,i,1, blocktype)
    mapStr += w    
    w = genString(2,i,3, blocktype)
    mapStr += w  
    w = genString(3,i,1, blocktype) #
    mapStr += w     
    w = genString(3,i,2, blocktype)
    mapStr += w         
    w = genString(1,i,1, blocktype) #
    mapStr += w
    w = genString(2,i,2, blocktype) #
    mapStr += w    
    w = genString(3,i,3, blocktype) #
    mapStr += w        
...
def get_action(obs, q_network, epsilon):
    with torch.no_grad():
        ...
        action_prob = [epsilon/5.0, epsilon/5.0, epsilon/5.0, epsilon/5.0, epsilon/5.0]
        actions = [0, 1, 2, 3, 4]
...
def get_observation(world_state):
    ...
    obs = np.zeros((30, OBS_SIZE, OBS_SIZE))
...
def log_returnsEpisode(num_episode, returns):     
        with open('returns2.txt', 'a') as f:
            f.write("{}\t{}\n".format(num_episode, returns[-1]))
...
def train(agent_host):
    ...
    while global_step < MAX_GLOBAL_STEPS:
        ...
        log_returnsEpisode(num_episode, returns)
